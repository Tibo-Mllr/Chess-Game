...
# retourne le nom de la première variable par ordre de création qui possède la valeur de var
            ...
            if event.type == MOUSEBUTTONDOWN:
                if event.button == 1:  # Si clic gauche
                    ...
                    if plateau[int(mouse[0]/80), int(mouse[1]/80)] == '':
                        ...
                        fenetre.blit(plateau[int(
                            mouse[0]/80), int(mouse[1]/80)], (int(mouse[0]/80)*80, int(mouse[1]/80)*80))
                        ...
                        while not event_happened:  # Tant que le déplacement de la pièce ne s'est pas produit, on attend
                            ...
                            if event.type == MOUSEBUTTONDOWN:
                                if event.button == 1:  # Si clic gauche
                                    mouse2 = event.pos
                                    plateau[(int(mouse2[0]/80), int(mouse2[1]/80))
                                            ] = plateau[(int(mouse[0]/80), int(mouse[1]/80))]
                                    plateau[(int(mouse[0]/80),
                                             int(mouse[1]/80))] = ''
                                    event_happened = True
